...
# ================= ================= =================
def generate_high_fidelity_3d(B, N, tp_in, d_in, bf_in, tw_in, tf_in, num_anchors, Y_length, tension_per_bolt, edge_g=2.5):
    fig = go.Figure()
    ...
    fig.add_trace(go.Mesh3d(
        ...
        y=[-tw_in/2, -tw_in/2, tw_in/2, tw_in/2, -tw_in/2, -tw_in/2, tw_in/2, tw_in/2],
        z=[tp_in, tp_in, tp_in, tp_in, tp_in+h_col, tp_in+h_col, tp_in+h_col, tp_in+h_col],
    ...
    # 5. พิกัดและการแสดงผลสลักเกลียวฝังแบบจำแนกสถานะแรง
    ...
    # 6. บล็อกลิ่มแรงกดคอนกรีตแบบ 3D Solid Prism
    if Y_length > 0:
...
with col_ctrl:
    ...
    length_weld_flange = 4.0 * bf_in  
    length_weld_web = 2.0 * (d_in - (2.0 * tf_in)) if (d_in - (2.0 * tf_in)) > 0 else 0.0
    ...
    weld_demand_flange = (p_u / (length_weld_flange + length_weld_web)) + (m_u / (bf_in * (d_in - tf_in))) if (length_weld_flange + length_weld_web) > 0 else 0.0
...
with col_view:
    ...
    util_plate = (t_req_min / tp_in) * 100 if tp_in > 0 else 0
    util_weld_flange = (weld_demand_flange / weld_capacity_per_inch) * 100 if weld_capacity_per_inch > 0 else 0
    util_weld_web = (weld_demand_web / weld_capacity_per_inch) * 100 if weld_capacity_per_inch > 0 else 0
    util_bolt_t = (tension_per_bolt / cap_tension_single) * 100 if cap_tension_single > 0 else 0
    util_bolt_v = (shear_per_bolt / cap_shear_single) * 100 if cap_shear_single > 0 else 0
